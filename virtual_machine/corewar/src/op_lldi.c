...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/11/05 01:35:02 by kdeloise          #+#    #+#             */
/*   Updated: 2020/11/05 01:35:02 by kdeloise         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	op_lldi(t_vm *vm, t_cursor *cursor)
{
	...
	cursor->reg[reg - 1] = bytecode_to_int32(vm->arena,
									(cursor->pc + addr1 + addr2), DIR_SIZE);
	cursor->carry = (t_bool)(!cursor->reg[reg - 1]);
	cursor->step += REG_LEN;
	if (vm->logs)
	{
