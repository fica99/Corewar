...
static t_hash		**asm_get_opers_hash_next(t_hash **opers_hash)
{
	...
	oper = asm_init_oper("sti", 0x0b, ARG_1_REG | ARG_2_ALL
		| (ARG_3_ALL & ~ARG_3_IND) | ARG_TYPE, 2);
	push_hash(opers_hash, "sti", (void *)oper, OPERS_SIZE);
	oper = asm_init_oper("fork", 0x0c, ARG_1_DIR, 2);
...
t_hash				**asm_get_opers_hash(void)
{
	...
	if (!(opers_hash = init_hash_table(OPERS_SIZE)))
		asm_sys_error();
	oper = asm_init_oper("live", 0x01, ARG_1_DIR, DIR_SIZE);
	push_hash(opers_hash, "live", (void *)oper, OPERS_SIZE);
	oper = asm_init_oper("ld", 0x02, ARG_1_DIR | ARG_1_IND
		| ARG_2_REG | ARG_TYPE, DIR_SIZE);
	push_hash(opers_hash, "ld", (void *)oper, OPERS_SIZE);
	oper = asm_init_oper("st", 0x03, ARG_1_REG | ARG_2_REG
		| ARG_2_IND | ARG_TYPE, DIR_SIZE);
	push_hash(opers_hash, "st", (void *)oper, OPERS_SIZE);
	oper = asm_init_oper("add", 0x04, ARG_1_REG | ARG_2_REG
		| ARG_3_REG | ARG_TYPE, DIR_SIZE);
	push_hash(opers_hash, "add", (void *)oper, OPERS_SIZE);
	oper = asm_init_oper("sub", 0x05, ARG_1_REG | ARG_2_REG
		| ARG_3_REG | ARG_TYPE, DIR_SIZE);
