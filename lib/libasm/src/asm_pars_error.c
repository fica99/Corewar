...
/*   Updated: 2020/11/05 22:58:31 by olegmulko        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (error == ASM_ERR_MIS_ARG)
		asm_fill_prms_from_token(token, prms, ERR_MIS_ARG);
	else if (error == ASM_ERR_MIS_NL)
	...
	t_asm_pars_prms *prms, t_asm_errors error)
{
	...
	else if (error == ASM_ERR_DUBL_LABEL)
		asm_fill_prms_from_token(token, prms, ERR_DUBL_LABEL);
